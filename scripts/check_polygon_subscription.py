...
def main():
    ...
    test_endpoint(client, "1. Daily Aggregates (Bars)", 
                  lambda: client.get_aggs("AAPL", 1, "day", "2025-09-15", "2025-09-20"))
    
    test_endpoint(client, "2. Active Tickers List",
                  lambda: client.get_tickers(market="stocks", active=True, limit=5))
    
    test_endpoint(client, "3. Earnings Calendar (Benzinga)",
                  lambda: client.get_earnings_calendar("AAPL", 
                                                      date(2025, 9, 1), 
                                                      date(2025, 9, 30)))
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"  ✅ Ticker details available")
