...
def benchmark_candidate_discovery():
    ...
    for limit in limits:
        ...
        print(f"  Limit {limit:3d}: {len(candidates):3d} candidates in {elapsed:.2f}s ({memory_used:.1f}MB)")
...
def benchmark_adv_prefiltering():
    ...
    for batch_size in batch_sizes:
        ...
        results.append({
            ...
            'output_candidates': len(prefiltered),
            'time': elapsed,
            'memory_mb': memory_used,
            'throughput': len(candidates) / elapsed if elapsed > 0 else 0
        })
        
        print(f"  Batch {batch_size:2d}: {len(candidates):3d}→{len(prefiltered):2d} in {elapsed:.2f}s ({memory_used:.1f}MB)")
...
def benchmark_spread_calculation():
    ...
    test_symbols = ["AAPL", "MSFT", "NVDA", "GOOGL", "AMZN", "TSLA", "META", "NFLX", "AMD", "INTC"]
    ...
    for num_symbols in [1, 3, 5, 10]:
        ...
        print(f"  {num_symbols:2d} symbols: {len(spreads):2d} successful in {elapsed:.2f}s ({memory_used:.1f}MB)")
...
def benchmark_full_pipeline():
    ...
    for target_size in target_sizes:
        ...
        print(f"  Target {target_size:3d}: {len(universe):3d} symbols in {elapsed:.1f}s ({memory_used:.1f}MB)")
...
def benchmark_memory_usage():
    ...
    print(f"    After discovery: {after_discovery:.1f} MB (+{after_discovery - initial_memory:.1f})")
    ...
    print(f"    After prefilter: {after_prefilter:.1f} MB (+{after_prefilter - after_discovery:.1f})")
    ...
    print(f"    After selection: {after_selection:.1f} MB (+{after_selection - after_prefilter:.1f})")
...
def benchmark_api_efficiency():
    ...
    for batch_size in [1, 2, 5]:
        ...
        print(f"  Batch {batch_size}: {api_calls} calls in {elapsed:.2f}s ({api_calls/elapsed:.1f} calls/s)")
