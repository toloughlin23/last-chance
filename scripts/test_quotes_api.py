...
def main():
    ...
    start_time = test_date.replace(hour=14, minute=30, second=0, microsecond=0)  # 9:30 AM ET
    end_time = test_date.replace(hour=21, minute=0, second=0, microsecond=0)    # 4:00 PM ET
    ...
    try:
        med_dollar, med_bps = client.median_spread_over_days(symbol, days=5, core_hours_only=True)
