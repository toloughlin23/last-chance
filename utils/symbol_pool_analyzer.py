...
class SymbolPoolAnalyzer:
    ...
    def analyze_symbol_performance(
        ...
        aggs_data = self.polygon_client.get_aggs(
            symbol, 1, "day", start_date, end_date, limit=200, adjusted=True, sort="asc"
        )
        ...
        avg_dollar_volume = (
            sum(dollar_volumes) / len(dollar_volumes) if dollar_volumes else 0
        )
        ...
        avg_atr_percent = (
            sum(atr_percentages) / len(atr_percentages) if atr_percentages else 0
        )
        ...
        if len(closes) > 1:
            ...
            price_stability = (
                1.0 / (1.0 + sum(price_changes) / len(price_changes))
                if price_changes
                else 0
            )
        else:
        ...
        liquidity_score = min(
            1.0, avg_dollar_volume / 100_000_000
        )  # Normalize to $100M
    ...
    def analyze_sp500_universe(
        self, analysis_days: int = 180, target_pool_size: int = 120
    ) -> List[SymbolMetrics]:
        """
    ...
    def _save_analysis_results(
        self, metrics: List[SymbolMetrics], analysis_days: int
    ) -> None:
