...
class ActiveUniverseProvider:
    ...
    def _get_fallback_candidates(self, limit: int) -> List[str]:
        ...
        known_large_caps = [
            ...
            "AVGO", "ORCL", "ADBE", "CRM", "CSCO", "ACN", "INTC", "AMD", 
            "QCOM", "TXN", "IBM", "INTU", "AMAT", "NOW", "MU", "LRCX",
            "KLAC", "SNPS", "CDNS", "ADSK", "FTNT", "PANW", "WDAY", "TEAM",
            
            # Financials  
            "BRK.B", "JPM", "V", "MA", "BAC", "WFC", "GS", "MS", "AXP",
            "SPGI", "BLK", "SCHW", "C", "PNC", "USB", "TFC", "COF", "BK",
            "ICE", "CME", "MCO", "MSCI", "TRV", "CB", "PGR", "ALL", "MET",
            
            # Healthcare
            "UNH", "JNJ", "LLY", "PFE", "ABBV", "MRK", "TMO", "ABT", "DHR",
            "CVS", "AMGN", "MDT", "BMY", "GILD", "ISRG", "VRTX", "SYK",
            "BSX", "CI", "HUM", "ELV", "ZTS", "REGN", "HCA", "MCK", "BDX",
            
            # Consumer
            "WMT", "PG", "HD", "COST", "MCD", "PEP", "KO", "NKE", "DIS",
            "SBUX", "TGT", "LOW", "TJX", "MDLZ", "MO", "PM", "CL", "EL",
            "GIS", "K", "KMB", "PGR", "STZ", "MNST", "KDP", "HSY", "CLX",
            
            # Industrials
            "CAT", "BA", "RTX", "HON", "UNP", "LMT", "DE", "UPS", "GE",
            "MMM", "CSX", "NOC", "GD", "EMR", "ETN", "ITW", "WM", "FDX",
            "NSC", "JCI", "PH", "CMI", "ROK", "OTIS", "CARR", "TT", "IR",
            
            # Energy & Materials
            "XOM", "CVX", "COP", "EOG", "SLB", "MPC", "PSX", "VLO", "PXD",
            "OXY", "HES", "KMI", "WMB", "LIN", "APD", "SHW", "ECL", "DD",
            "FCX", "NEM", "LYB", "DOW", "PPG", "ALB", "CTVA", "IFF", "CE",
            
            # Utilities & Real Estate
            "NEE", "SO", "DUK", "D", "AEP", "EXC", "SRE", "XEL", "WEC",
            "ES", "ED", "PEG", "AWK", "PCG", "EIX", "AMT", "PLD", "CCI",
            "EQIX", "PSA", "DLR", "O", "WELL", "SPG", "AVB", "EQR", "VTR",
            
            # Telecom & Media
            "VZ", "T", "TMUS", "CMCSA", "CHTR", "NFLX", "DIS", "WBD", "PARA",
            
            # Other large caps
            "BRK.A", "BKNG", "MELI", "SHOP", "UBER", "ABNB", "SQ", "PYPL",
            "FIS", "FISV", "ADP", "PAYX", "ZM", "DOCU", "OKTA", "VEEV",
        ]
        
        # Return up to limit symbols
        return known_large_caps[:limit]

    def _discover_and_rank_candidates(self, min_market_cap: float, analysis_days: int, 
                                    start_date: date, end_date: date, 
                                    max_candidates: int, batch_size: int) -> List[str]:
        ...
        try:
            ...
            data = self.polygon_client.get_tickers(market="stocks", active=True, limit=500)
            ...
            ranked_symbols = self._rank_symbols_by_quality(
                all_symbols, min_market_cap, analysis_days, 
                start_date, end_date, max_candidates, batch_size
            )
    ...
    def _rank_symbols_by_quality(self, symbols: List[str], min_market_cap: float, 
                               analysis_days: int, start_date: date, end_date: date,
                               max_candidates: int, batch_size: int) -> List[str]:
        ...
        for i in range(0, min(len(symbols), max_candidates * 2), batch_size):  # Analyze 2x more than needed
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = {}
                for symbol in batch:
                    future = executor.submit(self._analyze_symbol_quality, symbol, 
                                           min_market_cap, analysis_days, 
                                           start_date, end_date, quotes_client)
    ...
    def _analyze_symbol_quality(self, symbol: str, min_market_cap: float, 
                              analysis_days: int, start_date: date, end_date: date,
                              quotes_client) -> Optional[Dict]:
        ...
        try:
            ...
            price_data = self.polygon_client.get_aggs(
                symbol, 1, "day", start_date.isoformat(), 
                end_date.isoformat(), limit=analysis_days, adjusted=True
            )
    ...
    def _discover_candidates(self, min_market_cap: float = 10_000_000_000, limit: int = 500) -> List[str]:
        ...
        try:
            ...
            print(f"📈 Processing {len(all_symbols)} symbols for market cap filtering...")
    ...
    def _prefilter_by_adv_and_price(
        ...
        print(f"📈 Checking ADV for {len(symbols)} symbols in batches of {batch_size}...")
        ...
        for i in range(0, len(symbols), batch_size):
            ...
            print(f"  Batch {i//batch_size + 1}/{(len(symbols) + batch_size - 1)//batch_size}: symbols {i+1}-{batch_end}")
    ...
    def get_active_universe(
        ...
        try:
            ...
            print("ℹ️ Earnings calendar not available - proceeding without earnings exclusion")
