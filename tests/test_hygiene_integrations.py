...
@pytest.mark.skipif(
    not _have_polygon_key(),
    reason="POLYGON_API_KEY not set; see polygon/.env or project .env",
)
...
def test_earnings_filter_runs_without_error():
    ...
    out = h.filter_symbols(
        ["AAPL", "MSFT"], strategy_profile="mean_reversion", earnings_exclude=True
    )
    assert isinstance(out, list)
...
def test_ssr_filter_runs_without_error():
    h = Hygiene()
    out = h.filter_symbols(
        ["AAPL", "MSFT"], strategy_profile="mean_reversion", ssr_exclude=True
    )
    assert isinstance(out, list)
