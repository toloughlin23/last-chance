...
def make_polygon(
    price: float, prev_close: float, high: float, low: float, volume: int, vwap: float
):
    return {
        "status": "OK",
        "results": {
...
def test_ucbv_features_and_bounds():
    ...
    data = make_polygon(
        price=100.0, prev_close=99.5, high=101.0, low=99.0, volume=20000, vwap=100.0
    )
...
def test_ucbv_variation_high_vs_low_variance():
    ucbv = OptimizedInstitutionalUCBV()
    low_var = make_polygon(
        price=100.0, prev_close=100.0, high=100.4, low=99.6, volume=15000, vwap=100.0
    )
    high_var = make_polygon(
        price=100.0, prev_close=100.0, high=105.0, low=95.0, volume=30000, vwap=100.0
    )
...
def test_ucbv_update_changes_state():
    ucbv = OptimizedInstitutionalUCBV()
    data = make_polygon(
        price=100.0, prev_close=100.0, high=101.0, low=99.0, volume=25000, vwap=100.0
    )
