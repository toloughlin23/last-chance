...
def main():
    ...
    print(
        f"Training period: {config.training_start_date.date()} to {config.training_end_date.date()}"
    )
    print(
        f"Duration: {(config.training_end_date - config.training_start_date).days} days"
    )
