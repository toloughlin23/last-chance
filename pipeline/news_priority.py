...
def build_scores(
    symbols: List[str], lookback_hours: int = 16, per_symbol_limit: int = 25
) -> Dict[str, float]:
    client = NewsClient()
    since = (
    ...
    for sym in symbols:
        articles = client.fetch_symbol_news(
            sym, published_gte_utc=since, limit=per_symbol_limit, order="desc"
        )
...
def build_priority(
    ...
    scores = build_scores(
        symbols, lookback_hours=lookback_hours, per_symbol_limit=per_symbol_limit
    )
...
def save_priority_bundle(
    filepath: str, symbols: List[str], scores: Dict[str, float]
) -> None:
    os.makedirs(os.path.dirname(filepath), exist_ok=True)
    with open(filepath, "w", encoding="utf-8") as f:
...
def load_priority_bundle(filepath: str) -> Tuple[List[str], Dict[str, float]]:
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        ...
        return (
            syms if isinstance(syms, list) else [],
