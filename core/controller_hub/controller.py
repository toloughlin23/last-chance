...
class Phase1Controller:
    ...
    def run_cycle_and_publish(self, lookback_days: int = 5) -> Dict[str, float]:
        ...
        metrics = self.learning.run_learning_cycle(
            lookback_days=lookback_days, execute_trades=False
        )
        ...
        for symbol, data in market_data.items():
            enriched = (
                self.learning._safe_build_enriched(data)
                if hasattr(self.learning, "_safe_build_enriched")
                else None
            )
            if enriched is None:
        ...
        m = MetricsMessage(
            ...
            diversity_cross_algorithm=float(
                metrics.get("cross_algorithm_variance", 0.0)
            ),
